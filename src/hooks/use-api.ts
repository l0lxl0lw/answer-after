...
export function useContacts(filters: ContactFilters = {}, page = 1, perPage = 20) {
  ...
  return useQuery({
    queryKey: ['contacts', user?.organization_id, filters, page, perPage],
    queryFn: async () => {
...
export function useContactsByPhone(organizationId: string | undefined) {
  return useQuery({
    queryKey: ['contacts', 'by-phone', organizationId],
    queryFn: async () => {
      if (!organizationId) return new Map<string, Contact>();
