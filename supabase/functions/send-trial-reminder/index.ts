...
serve(async (req) => {
  ...
  try {
    logStep("Function started - checking for trials ending in 3 days");
    ...
    for (const subscription of subscriptions.data) {
      ...
      if (trialEnd >= threeDaysFromNowStart && trialEnd <= threeDaysFromNowEnd) {
        ...
        if (customer.deleted || !('email' in customer) || !customer.email) {
          logStep("Skipping - no email for customer", { customerId: subscription.customer });
          continue;
        }
        ...
        logStep("Email sent", { email: customer.email, subscriptionId: subscription.id });
        emailsSent++;
      }
    }
